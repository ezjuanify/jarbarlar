var EbbeAutoCSS = window.EbbeAutoCSS || null;

( function( $, api ) {

    EbbeAutoCSS = function(){
        this.values = {};
        this.lastValues = {};
        this.devices = [ 'desktop', 'tablet', 'mobile' ];
    };
    EbbeAutoCSS._change = false;
    EbbeAutoCSS.prototype.fonts = {};
    EbbeAutoCSS.prototype.styling_fields = {
        color: null,
        image: null,
        position: null,
        cover: null,
    ...
    EbbeAutoCSS.prototype.subsets = {};
    EbbeAutoCSS.prototype.variants = {};
    EbbeAutoCSS.prototype.media_queries = {
        all: '%s',
        desktop: '%s',
    ...
    EbbeAutoCSS.prototype.box_shadow_fields = {
        color:  null,
        x:  0,
        y:  0,
        blur:  0,
    ...
    EbbeAutoCSS.prototype.reset = function(){
        this.fonts = {};
        this.subsets = {};
        this.variants = {};
        this.css = {
    ...
    EbbeAutoCSS.prototype.encodeValue =function( value ){
        return encodeURI( JSON.stringify( value ) )
    };
    EbbeAutoCSS.prototype.decodeValue = function( value ){
        return JSON.parse( decodeURI( value ) );
    };

    EbbeAutoCSS.prototype.loop_fields = function( fields, values, skip_if_val_null, no_selector ){
    ...
    EbbeAutoCSS.prototype.run = function( setting_name ){
        ...
        if ( window.Ebbe_JS ) {
            if ( window.Ebbe_JS.css_media_queries ) {
                this.media_queries = window.Ebbe_JS.css_media_queries;
            }
        }
        this.reset();

        var that = this;
        that.loop_fields( Ebbe_Preview_Config.fields );
        ...
        if ( $( '#ebbe-google-font-css' ).length <= 0 ) {
            $( 'head' ).prepend( "<link rel='stylesheet' id='ebbe-google-font-css'  href='' type='text/css' media='all' />" );
        }

        if ( url ) {
            //css_code = "\r\n@import url('"+url+"');\r\n\r\n"+css_code;
            $( '#ebbe-google-font-css' ).attr( 'href', url );
        } else {
            $( '#ebbe-google-font-css' ).remove();
        }

        css_code = css_code.trim();
        if ( $( '#ebbe-style-inline-css' ).length <= 0 ) {
            $( 'head' ).append( "<style id='ebbe-style-inline-css' type='text/css'></style>" )
        }
        $( '#ebbe-style-inline-css' ).html( css_code );
    ...
    EbbeAutoCSS.prototype.get_setting = function(name, device, key  ){
        ...
        var value;
        var df = false;
        if ( !_.isUndefined( Ebbe_Preview_Config.fields['setting|'+name ] ) ) {
            var field = Ebbe_Preview_Config.fields['setting|'+name ];
    ...
    EbbeAutoCSS.prototype.get_google_fonts_url = function(){
    ...
    EbbeAutoCSS.prototype.join = function( object, glue ){
    ...
    EbbeAutoCSS.prototype.str_value = function( value, format, value_no_unit ){
        if ( _.isEmpty( value ) ) {
            return '';
        }
    ...
    EbbeAutoCSS.prototype.setup_color = function( value, format ){
        if ( format ) {
            if ( value ) {
    ...
    EbbeAutoCSS.prototype.setup_checkbox = function( value, format ){
        if ( format ) {
            if ( value ) {
                return format;
            }
    ...
    EbbeAutoCSS.prototype.setup_image = function( value, format ){
    ...
    EbbeAutoCSS.prototype.setup_slider = function ( value, format ){
        if ( ! _.isObject( value ) ) {
            value = {};
        }
    ...
    EbbeAutoCSS.prototype.setup_shadow = function ( value, format ){
        var that = this;
        if ( ! _.isObject( value ) ) {
            return '';
        }
    ...
    EbbeAutoCSS.prototype.setup_default = function( value, format ){
        if ( format ) {
            if ( value ) {
                return this.str_value( value, format );
            }
    ...
    EbbeAutoCSS.prototype.setup_css_ruler = function ( value, format ){
        if ( ! _.isObject( value ) ) {
            value = {};
        }
        value = _.defaults( value, {
    ...
    EbbeAutoCSS.prototype.setup_text_align = function( value, format ) {
        if ( format  ) {
            if ( value ) {
                return this.str_value( value, format );
            }
    ...
    EbbeAutoCSS.prototype.sanitize_color = function ( color ){
       return color;
    };

    EbbeAutoCSS.prototype.sanitize_slider = function ( value ){
        value = _.defaults( value, {
            unit: 'px',
            value: null
        });
    ...
    EbbeAutoCSS.prototype.sanitize_media = function ( value ) {
        if ( ! _.isObject( value ) ) {
            value = {};
        }
    ...
    EbbeAutoCSS.prototype.maybe_devices_setup = function( field, call_back, values, no_selector ) {
        var code = '';
        var code_array = {};
    ...
    EbbeAutoCSS.prototype.setup_font = function ( value ){
        if( ! _.isObject( value ) ) {
            value = {};
        }
        value = _.defaults( value, {
    ...
    EbbeAutoCSS.prototype.font = function( field, values ){
        var code = '';
        var that = this;
    ...
    EbbeAutoCSS.prototype.css_ruler = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_css_ruler', value, no_selector );
    };

    EbbeAutoCSS.prototype.shadow = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_shadow', value, no_selector );
    };

    EbbeAutoCSS.prototype.slider = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_slider', value, no_selector );
    };

    EbbeAutoCSS.prototype.color = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_color', value, no_selector );
    };

    EbbeAutoCSS.prototype.checkbox = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_checkbox', value, no_selector );
    };

    EbbeAutoCSS.prototype.image = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_image', value, no_selector );
    };

    EbbeAutoCSS.prototype.text_align = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_text_align', value, no_selector );
    };

    EbbeAutoCSS.prototype.setup_styling_fields = function( fields, list, selectors, type ){
        var that  = this;
        var newfs;
        var i;
        var newList = [];
    ...
    EbbeAutoCSS.prototype.styling = function( field ){
        var that = this;
        ...
        var listNormalFields = that.setup_styling_fields( normal_fields,  Ebbe_Preview_Config.styling_config.normal_fields, selectors, 'normal' );
        var listHoverFields = that.setup_styling_fields( hover_fields,  Ebbe_Preview_Config.styling_config.hover_fields, selectors, 'hover' );

        var listTabs = _.clone( Ebbe_Preview_Config.styling_config.tabs );
        if ( tabs === false ) {
    ...
    EbbeAutoCSS.prototype.modal = function ( field ){
        var that = this;
        var values = this.get_setting( field.name, 'all' );
        if ( ! _.isObject( values ) ) {
            values = {};
    ...
    EbbeAutoCSS.prototype.setup_font_style = function ( value ){
        if ( ! _.isObject( value ) ) {
            value = {};
        }
    ...
    EbbeAutoCSS.prototype.html_replace = function( field, value ){
    ...
    EbbeAutoCSS.prototype.html_class = function( field, v ){
        var value;
        if ( _.isUndefined( v ) ) {
            value = _.clone( v );
        } else {
    ...
    EbbeAutoCSS.prototype.typography = function( field, values ){
        ...
        var fields = {};
        var devices_css = {};
        _.each( Ebbe_Preview_Config.typo_fields, function( f ){
            fields[ f.name ] = f;
        } );
    ...
    var EbbeAutoCSSInit = new EbbeAutoCSS();

    api.bind( 'preview-ready', function() {
        //EbbeAutoCSSInit.run();
        EbbeAutoCSSInit.lastValues = api.get();
        EbbeAutoCSSInit.values = api.get();
    });
