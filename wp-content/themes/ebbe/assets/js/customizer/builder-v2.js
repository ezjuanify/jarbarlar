...
(function($) {
	...
	var is_rtl = Ebbe_Layout_Builder.is_rtl;
	...
	CustomizeBuilder_V2 = function(options, id) {
		var Builder = {
			...
			drag_drop: function() {
				...
				$(".ebbe--device-panel", that.container).each(function() {
					var panel = $(this);
					...
					$(".col-items, .ebbe-available-items", panel).each(function() {
			...
			addPanel: function(device) {
				...
				var template_id = "tmpl-ebbe--cb-panel-v2";
				...
				return (
					'<div class="ebbe--device-panel ebbe-vertical-panel ebbe--panel-' +
					device +
					'" data-device="' +
					device +
					'">' +
			...
			addDevicePanels: function() {
				...
				_.each(that.devices, function(device_name, device) {
					...
					$(".ebbe--cb-devices-switcher", that.container).append(
					...
					$(".ebbe--cb-body", that.container).append(panelHTML);
				});

				if ($("#ebbe-upsell-tmpl").length) {
					$($("#ebbe-upsell-tmpl").html()).insertAfter(
						$(".ebbe--cb-devices-switcher", that.container)
					);
				}
			},
			addItem: function(node) {
				var that = this;
				var template = that.getTemplate();
				var template_id = "tmpl-ebbe--cb-item";
				if ($("#" + template_id).length == 0) {
					return;
				}
			...
			addAvailableItems: function() {
				...
				_.each(that.devices, function(device_name, device) {
					var $itemWrapper = $(
						'<div class="ebbe-available-items" data-device="' + device + '"></div>'
					);

					$(".ebbe--panel-" + device, that.container).append(
						$itemWrapper
					);
			...
			switchToDevice: function(device, toggle_button) {
				...
				if (numberDevices > 1) {
					$(
						".ebbe--cb-devices-switcher a",
						that.container
					).removeClass("ebbe--tab-active");
					$(
						".ebbe--cb-devices-switcher .switch-to-" + device,
						that.container
					).addClass("ebbe--tab-active");
					$(".ebbe--device-panel", that.container).addClass(
						"ebbe--panel-hide"
					);
					$(
						".ebbe--device-panel.ebbe--panel-" + device,
						that.container
					).removeClass("ebbe--panel-hide");
			...
			addNewWidget: function( device, row_id, col_id, node, index) {
				...
				row = $( '.ebbe--cb-row.ebbe--row-'+row_id, panel );
				col = $( '.col-items.col-'+col_id, row );

				var $item = $( '.ebbe-available-items .grid-stack-item[data-id="'+node.id+'"]', panel );
			...
			focus: function() {
				this.container.on(
					"click",
					".ebbe--cb-item-setting, .ebbe--cb-item-name, .item-tooltip",
					function(e) {
						e.preventDefault();
			...
			remove: function() {
				...
				$document.on(
					"click",
					".ebbe--device-panel .ebbe--cb-item-remove",
					function(e) {
						e.preventDefault();
						var item = $(this).closest(".grid-stack-item");
						var panel = item.closest(".ebbe--device-panel");
						item.removeAttr("style");
						$(".ebbe-available-items", panel).append(item);
						that.save();
					}
				);
			},
			...
			save: function() {
				...
				_.each( that.devices, function( device_label, device ){
					data[ device ] = {};
					var devicePanel = $( '.ebbe--panel-'+device, that.container );
					$( '.ebbe--cb-row', devicePanel ).each( function(){
						var row = $( this );
			...
			showPanel: function() {
				var that = this;
				this.container
					.removeClass("ebbe--builder--hide")
					.addClass("ebbe--builder-show");
				setTimeout(function() {
			...
			hidePanel: function() {
				this.container.removeClass("ebbe--builder-show");
				$("#customize-preview")
			...
			togglePanel: function() {
				...
				wpcustomize.state("expandedPanel").bind(function(paneVisible) {
					if (wpcustomize.panel(options.panel).expanded()) {
						$document.trigger("ebbe_panel_builder_open", [
							options.panel
						]);
				...
				that.container.on("click", ".ebbe--panel-close", function(
					e
				) {
					e.preventDefault();
					that.container.toggleClass("ebbe--builder--hide");
					if (that.container.hasClass("ebbe--builder--hide")) {
			...
			init: function(controlId, items, devices) {
				...
				var template = that.getTemplate();
				var template_id = "tmpl-ebbe--builder-panel";
				var html = template(options, template_id);
				that.container = $(html);
				that.container.addClass( 'ebbe--panel-v2' );
				...
				that.container.on(
					"click",
					".ebbe--cb-devices-switcher a.switch-to",
					function(e) {
						e.preventDefault();
				...
				$document.trigger("ebbe_builder_panel_loaded", [id, that]);
