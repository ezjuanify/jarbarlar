(function($, wpcustomize) {
	...
	wpcustomize.bind("ready", function(e, b) {
		var addVersionChange = function(opts, builder, id, version) {
			if ( Ebbe_Layout_Builder.hide_switcher === 'yes' ) {
				return ;
			}
			...
			if (typeof opts.versions === "object") {
				...
				if (select_options) {
					var $select_options = $(
						'<span class="ebbe-swicth-version button-primary"><span>'+Ebbe_Layout_Builder.swicth_version+'</span><select class="builder-version-change ">' +
							select_options +
							"</select></span>"
					);
					$(".ebbe--cb-actions", builder.container).prepend(
						$select_options
					);
				}
				...
				$select_options.on("change", 'select', function() {
					var newVer = $(this).val();
					if ( ebbePanels[id + version] ) {
						ebbePanels[id + version].container.hide();
					}
					
					if (typeof ebbePanels[id + newVer] === "undefined") {
						var _builder;
						if ("v2" === newVer) {
					...
					if ( ebbePanels[id + newVer] ) {
						ebbePanels[id + newVer].container.show();
					}
		...
		_.each(Ebbe_Layout_Builder.builders, function(opts, id) {
			var builder;
			var version = "v1";
				...
				if (
					...
					$(
						...
						'<p class="footer_moved_widgets_text">' +
							Ebbe_Layout_Builder.footer_moved_widgets_text +
							"</p>"
					);
				}
			}, 1000);
		...
		wpcustomize.state("expandedSection").bind(function(section) {
			$(".ebbe--device-panel .grid-stack-item").removeClass(
				"item-active"
			);
			$(".ebbe--cb-row").removeClass("row-active");
			if (section) {
				$('.ebbe--cb-row[data-id="' + section.id + '"]').addClass(
					"row-active"
				);
				$(
					".ebbe--device-panel .grid-stack-item.for-s-" +
						section.id
		...
		function(e) {
			...
			if (template_name && template_name !== "") {
				...
				$.post(
					ajaxurl,
					{
						action: "ebbe_builder_save_template",
						name: input.val(),
	...
	$document.on("mouseover", ".ebbe--cb-row .grid-stack-item:not(.no-tooltip)", function(
		e
	) {
		var item = $(this);
		var nameW =
			$(".ebbe--cb-item-remove", item).outerWidth() +
			$(".ebbe--cb-item-setting", item).outerWidth();
	...
	$document.on("mouseleave", ".ebbe--cb-row .grid-stack-item:not(.no-tooltip)", function(
		e
	) {
