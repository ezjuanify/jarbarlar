		...
		var Builder = {
			...
			drag_drop: function() {
				...
				$(".ebbe--device-panel", that.container).each(function() {
					...
					var sortable_ids = [];
					that.panels[device] = {};
					$(".ebbe--cb-items", panel).each(function(index) {
					...
					$(
						".ebbe-available-items .grid-stack-item",
						panel
					).draggable({
						revert: "invalid",
						...
						start: function(event, ui) {
							...
							$(".ebbe--cb-items", panel).css("z-index", "");
			...
			addNewWidget: function($item, row) {
				...
				var panel = that.container.find(
					".ebbe--device-panel.ebbe--panel-" +
						that.activePanel
				);
				var el = row;
				if (!_.isObject(el)) {
					el = panel.find(".ebbe--cb-items").first();
				}
			...
			addPanel: function(device) {
				...
				var template_id = "tmpl-ebbe--cb-panel";
			...
			addAvailableItems: function() {
				...
				_.each(that.devices, function(device_name, device) {
					var $itemWrapper = $(
						'<div class="ebbe-available-items" data-device="' +
							device +
							'"></div>'
					);
					...
					_.each(device_data, function(items, row_id) {
						if (!_.isUndefined(items)) {
							_.each(items, function(node, index) {
								var item = $(
									'.ebbe-available-items[data-device="' +
										device +
