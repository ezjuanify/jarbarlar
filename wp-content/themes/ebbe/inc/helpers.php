 ...
 */
function ebbe_fallback_plugin_version($plugin_slug = ''){
	$plugins = array(
	    "modeltheme-framework" => "4.1",
	    "js_composer" => "7.6",
	    "revslider" => "6.7.3",
	);
 ...
 * @return string plugin version by slug
 */
function ebbe_plugin_version($plugin_slug = ''){
    ...
    if(($value = get_transient( $plugin_slug."_cache" )) === false) {
        ...
        return ebbe_fallback_plugin_version($plugin_slug);
    }
...
/* THEME DEFAULTS*/
if (!function_exists('ebbe_set_theme_defaults')) {
	function ebbe_set_theme_defaults() {
	    // Get Theme Defaults
	...
	if(! get_option('ebbe_theme_activated')) {
		//add_action('after_setup_theme', 'ebbe_set_theme_defaults');
		//add_option( 'ebbe_theme_activated', '1' );
	}
}
...
if (!function_exists('ebbe_body_class')) {
    function ebbe_body_class( $classes ) {
        // CHECK IF FEATURED IMAGE IS FALSE(Disabled)
        $post_featured_image = '';
        if (is_single()) {
            if (Ebbe()->get_setting('ebbe_blog_single_featured_image') == 0) {
        ...
        $is_nav_sticky = '';
        if (Ebbe()->get_setting( 'ebbe_is_nav_sticky')) {
            $is_nav_sticky = 'ebbe_is_nav_sticky';
        }else{
            $is_nav_sticky = '';
        }
        ...
        if (is_page()) {
            ...
            } else if (Ebbe()->get_setting( 'ebbe_general_settings_look_htransparent')) {
            	$is_transparent = 'is_transparent';
            }
        }else{
			if (Ebbe()->get_setting( 'ebbe_general_settings_look_htransparent')) {
            	$is_transparent = 'is_transparent';
            }
        }
    ...
    add_filter( 'body_class', 'ebbe_body_class' );
}
...
*/
if (!function_exists('ebbe_dynamic_css')) {
	add_action('wp_enqueue_scripts', 'ebbe_dynamic_css' );
	function ebbe_dynamic_css(){

		wp_register_style( 'ebbe-custom-style', false );
		wp_enqueue_style( 'ebbe-custom-style' );
	    ...
	    if (is_page()) {
			...
			if ((isset($mt_global_page_color) && $mt_global_page_color != '') && (isset($mt_global_page_color_hover) && $mt_global_page_color_hover != '')) {
				$ebbe_style_main_backgrounds_color = $mt_global_page_color;
				$ebbe_style_main_backgrounds_color_hover = $mt_global_page_color_hover;
			}else{
				$ebbe_style_main_backgrounds_color = Ebbe()->get_setting('ebbe_styling_settings_main_bg_color');
				$ebbe_style_main_backgrounds_color_hover = Ebbe()->get_setting('ebbe_styling_settings_main_bg_color_hover');
			}
    	...
    	if(Ebbe()->get_setting('ebbe_enable_parallax')) {
    		$html .= '.ebbe-breadcrumbs.ebbe-alignment-center {
		...
		@media only screen and (max-width: 767px) {
			body h1,
			body h1 span{
			    font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h1_size_mobile').' !important;
			    line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h1_lh_mobile').' !important;
			}
			body h2{
			    font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h2_size_mobile').' !important;
			    line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h2_lh_mobile').' !important;
			}
			body h3{
			    font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h3_size_mobile').' !important;
			    line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h3_lh_mobile').' !important;
			}
			body h4{
			    font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h4_size_mobile').' !important;
			    line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h4_lh_mobile').' !important;
			}
			body h5{
			    font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h5_size_mobile').' !important;
			    line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h5_lh_mobile').' !important;
			}
			body h6{
			    font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h6_size_mobile').' !important;
			    line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h6_lh_mobile').' !important;
			}
		}';
	    ...
	    @media only screen and (min-width: 768px) and (max-width: 1024px) {
	    	body h1,
	    	body h1 span{
	    		font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h1_size_tablet').' !important;
	    		line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h1_lh_tablet').' !important;
	    	}
	    	body h2{
	    		font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h2_size_tablet').' !important;
	    		line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h2_lh_tablet').' !important;
	    	}
	    	body h3{
	    		font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h3_size_tablet').' !important;
	    		line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h3_lh_tablet').' !important;
	    	}
	    	body h4{
	    		font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h4_size_tablet').' !important;
	    		line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h4_lh_tablet').' !important;
	    	}
	    	body h5{
	    		font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h5_size_tablet').' !important;
	    		line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h5_lh_tablet').' !important;
	    	}
	    	body h6{
	    		font-size: '.Ebbe()->get_setting('ebbe_styling_settings_h6_size_tablet').' !important;
	    		line-height: '.Ebbe()->get_setting('ebbe_styling_settings_h6_lh_tablet').' !important;
	    	}
	    }';
	    ...
	    $html .= '
                ...
                .wpb_button::after,.rotate45,.latest-posts .post-date-day,.latest-posts h3,.latest-tweets h3, .latest-videos h3,.page-template-template-blog .ebbe-article-inner .ebbe-more-link,button.vc_btn,.owl-theme .owl-controls .owl-page span,body .vc_btn.vc_btn-blue, body a.vc_btn.vc_btn-blue, body button.vc_btn.vc_btn-blue, .woocommerce input.button,table.compare-list .add-to-cart td a,.woocommerce #respond input#submit.alt, .woocommerce input.button.alt, .woocommerce nav.woocommerce-pagination ul li a:focus, body.woocommerce nav.woocommerce-pagination ul li a:hover, body.woocommerce .widget_price_filter .ui-slider .ui-slider-range, body.woocommerce nav.woocommerce-pagination ul li span.current, .widget_social_icons li a:hover, #subscribe > button[type="submit"],.prev-next-post a:hover .rotate45,.member-footer .social::before, .member-footer .social::after,.subscribe > button[type="submit"],.woocommerce #respond input#submit.alt.disabled, .woocommerce #respond input#submit.alt.disabled:hover, .woocommerce #respond input#submit.alt:disabled, .woocommerce #respond input#submit.alt:disabled:hover, .woocommerce #respond input#submit.alt[disabled]:disabled, .woocommerce #respond input#submit.alt[disabled]:disabled:hover, body.woocommerce button.button.alt.disabled, body.woocommerce button.button.alt.disabled:hover, .woocommerce button.button.alt:disabled, .woocommerce button.button.alt:disabled:hover, .woocommerce button.button.alt[disabled]:disabled, .woocommerce button.button.alt[disabled]:disabled:hover, .woocommerce input.button.alt.disabled, .woocommerce input.button.alt.disabled:hover, .woocommerce input.button.alt:disabled, .woocommerce input.button.alt:disabled:hover, .woocommerce input.button.alt[disabled]:disabled, .woocommerce input.button.alt[disabled]:disabled:hover,.no-results input[type="submit"],table.compare-list .add-to-cart td a,.shop_cart,h3#reply-title::after,.newspaper-info,.widget-title:after,.wpb_content_element .wpb_accordion_wrapper .wpb_accordion_header.ui-state-active,#primary .main-content ul li:not(.rotate45)::before,.menu-search .btn.btn-primary:hover,.btn-register, .modeltheme-modal input[type="submit"], .modeltheme-modal button[type="submit"], form#login .register_button, form#login .submit_button, .widget_address_social_icons .social-links a,.hover-components .component:hover,.nav-previous a, .nav-next a,article.dokan-orders-area .dokan-panel-default > .dokan-panel-heading,#signup-modal-content .woocommerce-form-register.register .button[type="submit"],body .woocommerce-MyAccount-navigation-link > a,.newsletter-footer input.submit:hover, .newsletter-footer input.submit:focus, a.remove-wsawl.sa-watchlist-action,.wcv-dashboard-navigation li a,.wc_vendors_active form input[type="submit"], #wp-calendar td#today,.wishlist-title-with-form .show-title-form,.categories_shortcode .category, body .pagination .page-numbers.current,body .pagination .page-numbers:hover,.category-button.boxed a, .woocommerce div.product .woocommerce-tabs ul.tabs li.active,body .tp-bullets.preview1 .bullet,.sale_banner_right span.read-more {
                        background: '.esc_html($ebbe_style_main_backgrounds_color).'; 
                }
                ...
                .author-bio,body blockquote,.widget_popular_recent_tabs .nav-tabs > li.active,body .left-border, body .right-border,body .member-header,body .member-footer .social,.woocommerce div.product .woocommerce-tabs ul.tabs li.active, body .button[type="submit"],.wpb_content_element .wpb_tabs_nav li.ui-tabs-active,.header_mini_cart,.header_mini_cart.visible_cart,#contact-us .form-control:focus,.header_mini_cart .woocommerce .widget_shopping_cart .total, .header_mini_cart .woocommerce.widget_shopping_cart .total,.sale_banner_holder:hover, .woocommerce.single-product div.product.product-type-auction form.cart .button.single_add_to_cart_button,body .sidebar-content .widget-title::before, .sidebar-content .widget h2::before, .sidebar-content .widget .wp-block-search__label::before {
                        border-color: '.Ebbe()->get_setting('ebbe_styling_settings_main_border_color').' !important;
                }
                .woocommerce #respond input#submit:hover, .wc_vendors_active form input[type="submit"]:hover,.wcv-dashboard-navigation li a:hover,.woocommerce input.button:hover,table.compare-list .add-to-cart td a:hover,.woocommerce #respond input#submit.alt:hover, .woocommerce input.button.alt:hover, .latest-posts .post-date-month,.button.solid-button:hover,body .vc_btn.vc_btn-blue:hover, body a.vc_btn.vc_btn-blue:hover, body button.vc_btn.vc_btn-blue:hover,.subscribe > button[type="submit"]:hover,table.compare-list .add-to-cart td a:hover,.shop_cart:hover,.widget_address_social_icons .social-links a:hover,.page-template-template-blog .ebbe-article-inner .ebbe-more-link:hover,form#login .submit_button:hover,.modeltheme-modal input[type="submit"]:hover, .modeltheme-modal button[type="submit"]:hover, .modeltheme-modal p.btn-register-p a:hover,#signup-modal-content .woocommerce-form-register.register .button[type="submit"]:hover, .no-touch #cd-zoom-in:hover, .no-touch #cd-zoom-out:hover,.woocommerce .woocommerce-widget-layered-nav-dropdown__submit:hover, .pagination > li > a.current, .pagination > li > a:hover {
                        background: '.esc_html($ebbe_style_main_backgrounds_color_hover).';
                }
				::selection{
                    color: '.Ebbe()->get_setting('ebbe_styling_settings_selection_color').';
                    background: '.Ebbe()->get_setting('ebbe_styling_settings_selection_bg').';
				}
                ...
                #navbar .sub-menu, .navbar ul li ul.sub-menu {
                        background-color: '.Ebbe()->get_setting('ebbe_styling_settings_submenu_background').';
                }
                #navbar ul.sub-menu li a {
                        color: '.Ebbe()->get_setting('ebbe_styling_settings_submenu_text_color').';
                }
                #navbar ul.sub-menu li a:hover {
                        color: '.Ebbe()->get_setting('ebbe_styling_settings_submenu_background_hover').';
                }
                ';
	    wp_add_inline_style( 'ebbe-custom-style', ebbe_minify_css($html) );
	}
}