...
class Ebbe_Font_Icons
{
	...
	function get_icons()
	{
		if (is_null(self::$_icons)) {
			$ver = get_option('ebbe_fa_ver', 'v4');

			$icons = [];

			$icons['font-awesome-v456'] = array(
				'name'         => esc_html__('Font Awesome v6', 'ebbe'),
			...
			$icons['linear-icons-v100'] = array(
				'name'         => esc_html__('Linear Icons (Free)', 'ebbe'),
			...
			self::$_icons = apply_filters('ebbe/customizer/font_icons', $icons);
		}
	...
	function is_used()
	{
		...
		$check = apply_filters('ebbe/load-icons', true);
		if (is_null($check)) {
			$builders = array('header_builder_panel', 'footer_builder_panel');
			$list     = apply_filters('ebbe/icon_used', array());

			foreach ($builders as $setting_key) {
				$data = Ebbe()->get_setting($setting_key);
				if (is_array($data)) {
