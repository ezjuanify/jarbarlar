...
class Ebbe_Customizer_Control_Base extends WP_Customize_Control {
	/**
	 * The control type.
	 *
	 * @access public
	...
	public function to_json() {
		...
		$this->json['_pro']           = class_exists( 'Ebbe_Pro' );
		...
		if ( 'repeater' == $this->setting_type ) {
			$this->json['l10n']             = array(
				'untitled' => esc_html__( 'Untitled', 'ebbe' ),
			);
	...
	function merge_items( $destination = array(), $new_array = array() ) {
		return Ebbe_Customizer::merge_items( $destination, $new_array );
	}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'ebbe-customizer-control', esc_url( get_template_directory_uri() ) . '/assets/css/admin/customizer/customizer' . $suffix . '.css' ); // phpcs:ignore
		wp_enqueue_style( 'select2', esc_url( get_template_directory_uri() ) . '/assets/css/admin/select2' . $suffix . '.css' ); // phpcs:ignore
		wp_enqueue_script( 'ebbe-color-picker-alpha', esc_url( get_template_directory_uri() ) . '/assets/js/customizer/color-picker-alpha' . $suffix . '.js', array( 'wp-color-picker' ), false, true ); // phpcs:ignore
		...
		if ( is_null( self::$_args_loaded ) ) {
			...
			$args = array(
				...
				'is_rtl'           => is_rtl(),
				'theme_default'    => esc_html__( 'Theme Default', 'ebbe' ),
				'reset'            => esc_html__( 'Reset this section settings', 'ebbe' ),
				'untitled'         => esc_html__( 'Untitled', 'ebbe' ),
				'confirm_reset'    => esc_html__( 'Do you want to reset this section settings?', 'ebbe' ),
				'list_font_weight' => array(
					''       => esc_html__( 'Default', 'ebbe' ),
					'normal' => _x( 'Normal', 'ebbe-font-weight', 'ebbe' ),
					'bold'   => _x( 'Bold', 'ebbe-font-weight', 'ebbe' ),
				),
				'typo_fields'      => Ebbe()->customizer->get_typo_fields(),
				'styling_config'   => Ebbe()->customizer->get_styling_config(),
				'devices'          => Ebbe()->customizer->devices,
			...
			wp_localize_script( 'ebbe-customizer-control', 'Ebbe_Control_Args', apply_filters( 'Ebbe_Control_Args', $args ) ); // phpcs:ignore

			$color_picker_l10n_args = array(
				'clear'            => esc_html__( 'Clear', 'ebbe' ),
				'clearAriaLabel'   => esc_html__( 'Clear color', 'ebbe' ),
				'defaultString'    => esc_html__( 'Default', 'ebbe' ),
				'defaultAriaLabel' => esc_html__( 'Select default color', 'ebbe' ),
				'pick'             => esc_html__( 'Select Color', 'ebbe' ),
				'defaultLabel'     => esc_html__( 'Color value', 'ebbe' ),
			);
			wp_localize_script( 'wp-color-picker', 'Ebbe_ColorPicker_L10n', apply_filters( 'Ebbe_Control_Color_Picker_L10n_Args', $color_picker_l10n_args ) ); // phpcs:ignore
	...
	protected function render() {
		...
		?>
		<li id="<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $class ); ?><?php echo esc_attr( $this->device ) ? '  ebbe--device-show ebbe--device-' . esc_attr( $this->device ) : ''; ?>">
	...
	protected function render_content() {
		...
		if ( 'heading' == $this->setting_type ) {
			?>
			<div class="ebbe-control--heading">
				<label>
		...
		?>
		<div class="ebbe--settings-wrapper <?php echo ( isset( $this->description ) && ! empty( $this->description ) ) ? 'show-short-desc' : ''; ?> <?php echo esc_attr( $this->label || $this->description ) ? 'has-title-desc' : 'no-title-desc'; ?>">
			<?php if ( $this->label ) { ?>
				<div data-control="<?php echo esc_attr( $this->id ); ?>" class="ebbe-control-field-header ebbe-field-heading">
					<label>
						<?php if ( ! empty( $this->label ) ) : ?>
							<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
						<?php endif; ?>
			...
			<?php if ( 'custom_html' != $this->setting_type ) { ?>
				<div class="ebbe--settings-fields<?php echo ( 'repeater' == $this->setting_type ) ? ' ebbe--repeater-items' : ''; ?>"></div>
				<?php if ( 'repeater' == $this->setting_type ) { ?>
					<div class="ebbe--repeater-actions">
						<a href="#" class="ebbe--repeater-reorder" data-text="<?php esc_attr_e( 'Reorder', 'ebbe' ); ?>" data-done="<?php _e( 'Done', 'ebbe' ); ?>"><?php _e( 'Reorder', 'ebbe' ); ?></a>
						<?php if ( false !== $this->addable ) { ?>
							<button type="button" class="button ebbe--repeater-add-new"><?php _e( 'Add an item', 'ebbe' ); ?></button>
						<?php } ?>
					</div>
				<?php } ?>
			<?php } ?>
	...
	static function before_field() {
		...
		#>
		<div class="ebbe--field ebbe--field-{{ field.type }} {{ field.class }} ebbe--field-name-{{ field.original_name }}" data-required="{{ required }}" data-field-name="{{ field.name }}">
		<?php
	}
	...
	static function field_header() {
		...
		<div class="ebbe-field-header">
			<# if ( field.label ) { #>
			<div class="ebbe-field-heading">
	...
	static function field_template() {
		?>
		<script type="text/html" id="tmpl-field-ebbe-text">
			<?php
			self::before_field();
			?>
			<?php echo self::field_header(); ?>
			<div class="ebbe-field-settings-inner">
				<input type="{{ field.type }}" class="ebbe-input ebbe-only" placeholder="{{ field.placeholder }}" data-name="{{ field.name }}" value="{{ field.value }}">
			</div>
			<?php
			self::after_field();
			?>
