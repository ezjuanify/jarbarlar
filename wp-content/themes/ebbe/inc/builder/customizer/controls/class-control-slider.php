<?php
class Ebbe_Customizer_Control_Slider extends Ebbe_Customizer_Control_Base {
	static function field_template() {
		echo '<script type="text/html" id="tmpl-field-ebbe-slider">';
		self::before_field();
		?>
		<#
		...
		<div class="ebbe-field-settings-inner">
			<div class="ebbe-input-slider-wrapper">
				<div class="ebbe--css-unit">
					...
					<a href="#" class="reset" title="<?php esc_attr_e( 'Reset', 'ebbe' ); ?>"></a>
				</div>
				<div data-min="{{ field.min }}" data-default="{{ JSON.stringify( field.default ) }}" data-step="{{ field.step }}" data-max="{{ field.max }}" class="ebbe-input-slider"></div>
				<input type="number" min="{{ field.min }}" step="{{ field.step }}" max="{{ field.max }}" class="ebbe--slider-input ebbe-input" data-name="{{ field.name }}-value" value="{{ field.value.value }}" size="4">
			</div>
		</div>
		<?php
		self::after_field();
