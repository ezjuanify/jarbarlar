...
class Ebbe_Builder_Item_Primary_Menu {
	public $id = 'primary-menu';

	function item() {
		return array(
			'name'    => esc_html__( 'Primary Menu', 'ebbe' ),
			'id'      => $this->id,
			'width'   => '6',
	...
	function customize() {
		...
		$config  = array(
			array(
				...
				'theme_supports' => '',
				'title'          => esc_html__( 'Primary Menu', 'ebbe' ),
				'description'    => sprintf( __( 'Assign <a href="#menu_locations"  class="focus-section">Menu Location</a> for %1$s', 'ebbe' ), esc_html__( 'Primary Menu', 'ebbe' ) ),
			),

			array(
				'name'            => 'ebbe_primary_menu_style',
				'type'            => 'image_select',
				'section'         => $section,
			...
			array(
				'name'       => 'ebbe_primary_menu_style_border_h',
				'type'       => 'slider',
				'section'    => $section,
				'selector'   => 'format',
				'max'        => 20,
				'title'      => __( 'Border Height', 'ebbe' ),
			...
			array(
				'name'       => 'ebbe_primary_menu_style_border_pos',
				'type'       => 'slider',
				'section'    => $section,
				'selector'   => 'format',
				'min'        => - 50,
			...
			array(
				'name'       => 'ebbe_primary_menu_style_border_color',
				'type'       => 'color',
				'section'    => $section,
				'selector'   => 'format',
			...
			array(
				'name'    => 'ebbe_primary_menu_top_heading',
				'type'    => 'heading',
				'section' => $section,
				'title'   => esc_html__( 'Menu', 'ebbe' ),
			),
			array(
				'name'        => 'ebbe_primary_menu_typography',
				'type'        => 'typography',
				'section'     => $section,
	 ...
	 */
	function render() {
		$style = sanitize_text_field( Ebbe()->get_setting( 'ebbe_primary_menu_style' ) );
		if ( $style ) {
			$style = sanitize_text_field( $style );
		}

		$hide_arrow = sanitize_text_field( Ebbe()->get_setting( 'ebbe_primary_menu__hide-arrow' ) );
		if ( $hide_arrow ) {
		...
		if ( has_nav_menu( 'hb-primary-navigation' ) ) {
			...
			echo '<p class="no-menu">';
                echo esc_html__('Primary navigation menu is missing.', 'ebbe');
            echo '</p>';
		}
 ...
 * @return mixed
 */
function ebbe_change_nav_menu_item_id( $string_id, $item, $args ) {
...
add_filter( 'nav_menu_item_id', 'ebbe_change_nav_menu_item_id', 55, 3 );
 ...
 * @return string
 */
function ebbe_add_icon_to_menu( $title, $item, $args, $depth ) {
...
add_filter( 'nav_menu_item_title', 'ebbe_add_icon_to_menu', 25, 4 );


// Register header item.
Ebbe_Customize_Layout_Builder()->register_item( 'header', new Ebbe_Builder_Item_Primary_Menu() );
