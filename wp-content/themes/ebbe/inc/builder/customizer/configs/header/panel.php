...
class Ebbe_Builder_Header extends Ebbe_Customize_Builder_Panel {
	public $id = 'header';
	...
	function get_config() {
		return array(
			'id'            => $this->id,
			'title'         => esc_html__( 'Header Builder', 'ebbe' ),
			...
			'versions'      => array(
				'v1' => array(
					...
					'label' => esc_html__( 'Version 1', 'ebbe' ),
				),
				'v2' => array(
					'control_id' => 'header_builder_panel_v2',
					'label' => esc_html__( 'Version 2', 'ebbe' ),
				),
			),
			'devices'       => array(
				'desktop'      => esc_html__( 'Desktop', 'ebbe' ),
				'mobile'       => esc_html__( 'Mobile/Tablet', 'ebbe' ),
			),
		);
	}
	...
	function get_rows_config() {
		return array(
			'top'     => esc_html__( 'Header Top', 'ebbe' ),
			'main'    => esc_html__( 'Header Main', 'ebbe' ),
			'bottom'  => esc_html__( 'Header Bottom', 'ebbe' ),
			'sidebar' => esc_html__( 'Menu Sidebar', 'ebbe' ),
		);
	}
	...
	function row_config( $section = false, $section_name = false ) {
		...
		if ( ! $section_name ) {
			$section_name = esc_html__( 'Header Top', 'ebbe' );
		}
		...
		$config = array(
			...
			array(
				...
				'title'           => esc_html__( 'Container Width', 'ebbe' ),
			),
			array(
			...
			array(
				...
				'type'             => 'styling',
				'section'          => $section,
				'title'            => esc_html__( 'Advanced Styling', 'ebbe' ),
				'description'      => sprintf( __( 'Advanced styling for %s', 'ebbe' ), $section_name ),
			...
			array(
				...
				'section'          => $section,
				'default'          => '1',
				'title'            => esc_html__( 'Visible on Sticky Header?', 'ebbe' ),
				'checkbox_label' => esc_html__( 'Show/Hide', 'ebbe' ),
				'description'      => esc_html__( '*If sticky header is enabled', 'ebbe' ),
...
if ( ! function_exists( 'ebbe_header_layout_settings' ) ) {
	function ebbe_header_layout_settings( $item_id = '', $section = '', $cb = '', $name_prefix = 'header_' ) {

		if ( ! $cb ) {
			$cb = 'ebbe_customize_render_header';
		}
		...
		if ( ! $name_prefix ) {
			...
			if ( strpos( $item_id, 'footer' ) !== false ) {
				$class       = '.footer--row';
				$name_prefix = 'footer_';
				$cb          = 'ebbe_customize_render_footer';
			}
		}
		...
		$layout = array(
			array(
				...
				'priority' => 800,
				'section'  => $section,
				'title'    => esc_html__( 'Item Layout', 'ebbe' ),
			),
...
Ebbe_Customize_Layout_Builder()->register_builder( 'header', new Ebbe_Builder_Header() );
