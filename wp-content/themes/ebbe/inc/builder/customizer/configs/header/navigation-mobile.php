...
class Ebbe_Builder_Item_Navigation_Mobile {
	public $id = 'navigation_mobile';

	function item() {
		return array(
			'name'    => esc_html__( 'Mobile Navigation', 'ebbe' ),
			'id'      => $this->id,
			'width'   => '6',
	...
	function customize() {
		...
		$config  = array(
			array(
				...
				'theme_supports' => '',
				'title'          => esc_html__( 'Mobile Navigation', 'ebbe' ),
				'description'    => sprintf( __( 'Assign <a href="#menu_locations"  class="focus-section">Menu Location</a> for %1$s', 'ebbe' ), esc_html__( 'Mobile Navigation', 'ebbe' ) ),
			),

			array(
				'name'            => 'ebbe_navigation_mobile_style',
				'type'            => 'image_select',
				'section'         => $section,
				'selector'        => '.builder-item--' . $this->id . " .{$this->id}",
				'render_callback' => $fn,
				'title'           => esc_html__( 'Menu Preset', 'ebbe' ),
				'default'         => 'style-plain',
			...
			array(
				'name'       => 'ebbe_navigation_mobile_style_border_h',
				'type'       => 'slider',
				'section'    => $section,
				'selector'   => 'format',
				'max'        => 20,
				'title'      => esc_html__( 'Border Height', 'ebbe' ),
			...
			array(
				'name'       => 'ebbe_navigation_mobile_style_border_pos',
				'type'       => 'slider',
				'section'    => $section,
				'selector'   => 'format',
				'min'        => - 50,
				'max'        => 50,
				'title'      => esc_html__( 'Border Position', 'ebbe' ),
			...
			array(
				'name'       => 'ebbe_navigation_mobile_style_border_color',
				'type'       => 'color',
				'section'    => $section,
				'selector'   => 'format',
			...
			array(
				'name'           => 'ebbe_navigation_mobile__hide-arrow',
				'type'           => 'checkbox',
				'section'        => $section,
				'selector'       => '.builder-item--' . $this->id . " .{$this->id}",
				'checkbox_label' => esc_html__( 'Hide menu dropdown arrow', 'ebbe' ),
				'css_format'     => 'html_class',
			),

			array(
				'name'            => 'ebbe_navigation_mobile_arrow_size',
				'type'            => 'slider',
				'devices_setting' => true,
				'section'         => $section,
				'selector'        => 'format',
				'max'             => 20,
				'title'           => esc_html__( 'Arrow icon size', 'ebbe' ),
				'css_format'      => ".builder-item--navigation_mobile .nav-icon-angle { width: {{value}}; height: {{value}}; }",
				'required'        => array( 'ebbe_navigation_mobile__hide-arrow', '!=', 1 ),
			),

			array(
				'name'    => 'ebbe_navigation_mobile_top_heading',
				'type'    => 'heading',
				'section' => $section,
				'title'   => esc_html__( 'Top Menu', 'ebbe' ),
			),

			array(
				'name'        => 'ebbe_navigation_mobile_item_styling',
				'type'        => 'styling',
				'section'     => $section,
				'title'       => esc_html__( 'Items Styling', 'ebbe' ),
				'description' => esc_html__( 'Styling level menu items', 'ebbe' ),
				'selector'    => array(
				...
				'fields'      => array(
					'tabs'          => array(
						...
						'hover'  => esc_html__( 'Hover/Active', 'ebbe' ),
					),
					'normal_fields' => array(
						'link_color'    => false,
						'bg_cover'      => false,
			...
			array(
				'name'        => 'ebbe_navigation_mobile_typography',
				'type'        => 'typography',
				'section'     => $section,
				'title'       => esc_html__( 'Menu Items Typography', 'ebbe' ),
				'description' => esc_html__( 'Typography for menu', 'ebbe' ),
		...
		$config = apply_filters( 'ebbe/customize-menu-config-more', $config, $section, $this );

		// Item Layout.
		return array_merge( $config, ebbe_header_layout_settings( $this->id, $section ) );
	}
	 ...
	 */
	function render() {
		$style = sanitize_text_field( Ebbe()->get_setting( 'ebbe_navigation_mobile_style' ) );
		if ( $style ) {
			$style = sanitize_text_field( $style );
		}

		$hide_arrow = sanitize_text_field( Ebbe()->get_setting( 'ebbe_navigation_mobile__hide-arrow' ) );
		if ( $hide_arrow ) {
 ...
 * @return mixed
 */
function ebbe_change_nav_menu_item_id_navigation_mobile( $string_id, $item, $args ) {
...
add_filter( 'nav_menu_item_id', 'ebbe_change_nav_menu_item_id_navigation_mobile', 55, 3 );


// Register header item.
Ebbe_Customize_Layout_Builder()->register_item( 'header', new Ebbe_Builder_Item_Navigation_Mobile() );
