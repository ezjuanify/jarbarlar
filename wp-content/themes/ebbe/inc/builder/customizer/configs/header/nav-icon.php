...
class Ebbe_Builder_Item_Nav_Icon {
	public $id = 'nav-icon';

	function item() {
		return array(
			'name'    => esc_html__( 'Menu Icon', 'ebbe' ),
			'id'      => $this->id,
			'width'   => '3',
	...
	function customize() {
		...
		$config   = array(
			array(
				...
				'theme_supports' => '',
				'title'          => esc_html__( 'Menu Icon', 'ebbe' ),
			),
			array(
				'name'            => 'ebbe_nav_icon_size',
				'type'            => 'radio_group',
				'section'         => 'header_menu_icon',
				'selector'        => $selector,
				'render_callback' => $fn,
			...
			array(
				'name'       => 'ebbe_nav_icon_item_color_hover',
				'type'       => 'color',
				'section'    => 'header_menu_icon',
				'css_format' => 'color: {{value}};',
		...
		// Item Layout.
		return array_merge( $config, ebbe_header_layout_settings( $this->id, 'header_menu_icon' ) );
	}

	function render() {
		$style      = sanitize_text_field( Ebbe()->get_setting( 'ebbe_nav_icon_style' ) );
		$sizes      = Ebbe()->get_setting( 'ebbe_nav_icon_size', 'all' );
...
Ebbe_Customize_Layout_Builder()->register_item( 'header', new Ebbe_Builder_Item_Nav_Icon() );
