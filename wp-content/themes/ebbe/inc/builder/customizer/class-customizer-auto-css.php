...
class Ebbe_Customizer_Auto_CSS {
	 ...
	 * Get intance.
	 *
	 * @return Ebbe_Customizer_Auto_CSS
	 */
	...
	function setup_shadow( $value, $format ) {
		...
		$value    = Ebbe_Sanitize_Input::sanitize_color( $p['color'] );
		$color    = $value;
	...
	function setup_color( $value, $format ) {
		$value = Ebbe_Sanitize_Input::sanitize_color( $value );
		if ( $format ) {
	...
	function styling( $field, $values = null ) {
		$values = Ebbe()->get_setting( $field['name'], 'all' );
		...
		if ( isset( $values['normal']['bg_image'] ) ) {
			$image = Ebbe()->get_media( $values['normal']['bg_image'] );
			if ( ! $image ) {
		...
		foreach ( Ebbe()->customizer->devices as $device ) {
			$css = '';
			if ( isset( $selectorCSSDevices[ $device ] ) ) {
	...
	function maybe_devices_setup( $field, $call_back, $values = null, $no_selector = false ) {
		...
		if ( isset( $field['device_settings'] ) && $field['device_settings'] ) {
			...
			foreach ( Ebbe()->customizer->devices as $device ) {
				$value = null;
				if ( is_null( $values ) ) {
					$value = Ebbe()->get_setting( $field['name'], $device );
				} else {
			...
			if ( is_null( $values ) ) {
				$values = Ebbe()->get_setting( $field['name'] );
			}
			if ( method_exists( $this, $call_back ) ) {
		...
		$code_array = apply_filters( 'ebbe/customizer/auto_css', $code_array, $field, $this );
	...
	function typography( $field ) {
		...
		$fields      = array();
		$devices_css = array();
		foreach ( Ebbe()->customizer->get_typo_fields() as $f ) {
			$fields[ $f['name'] ] = $f;
		}
		...
		$devices_css = apply_filters( 'ebbe/customizer/auto_css', $devices_css, $field, $this );
	...
	public function render_css( $fields = array() ) {
		...
		$css_code = apply_filters( 'ebbe/auto-css', $css_code, $this );
		return $css_code;
	}
	...
	function auto_css( $partial = false ) {
		...
		$config_fields = Ebbe()->customizer->get_config();
		/**
