 ...
 * @since 0.2.9
 */
abstract class Ebbe_Abstract_Layout_Frontend {
	...
	function get_settings() {
		if ( $this->data ) {
			if (!is_customize_preview()) {
				if(!empty(get_option('Ebbe_header_saved_templates')[ebbe_get_page_header_template()]['data']['header_builder_panel_v2'])){
			        $custom_header_template = get_option('Ebbe_header_saved_templates')[ebbe_get_page_header_template()]['data']['header_builder_panel_v2'];
	...
	public function close_icon( $class = '' ) {
		$menu_sidebar_skin = Ebbe()->get_setting( 'header_sidebar_text_mode' );
		$close             = '<a class="close is-size-medium ' . $menu_sidebar_skin . esc_attr( $class ) . '" href="#">
        <span class="hamburger hamburger--squeeze is-active">
            <span class="hamburger-box">
              <span class="hamburger-inner"><span class="screen-reader-text">' . esc_html__( 'Menu', 'ebbe' ) . '</span></span>
            </span>
        </span>
        <span class="screen-reader-text">' . esc_html__( 'Close', 'ebbe' ) . '</span>
        </a>';
