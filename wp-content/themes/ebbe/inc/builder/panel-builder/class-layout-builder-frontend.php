 ...
 * @since 0.2.9
 */
class Ebbe_Layout_Builder_Frontend extends Ebbe_Abstract_Layout_Frontend {
	...
	function get_settings() {
		...
		$data = Ebbe()->get_setting( $this->control_id );
		$data = wp_parse_args(
			$data,
			array(
				'desktop' => '',
				...
				if ( is_array( $row_items ) && count( $row_items ) ) {
					...
					foreach ( $row_items as $item_index => $item ) {
						...
						if ( ! $items[ $item['id'] ] ['render_content'] ) {
							 ...
							 * @since 0.2.1
							 */
							do_action( 'ebbe/builder/' . $this->id . '/before-item/' . $item['id'] );
							$object_item = Ebbe_Customize_Layout_Builder()->get_builder_item( $this->id, $item['id'] );
							 ...
							 * @since 0.2.1
							 */
							do_action( 'ebbe/builder/' . $this->id . '/after-item/' . $item['id'] );
	...
	public function render_row( $items, $id = '', $device = 'desktop' ) {
		...
		while ( $index < $n ) {
			...
			$merge      = Ebbe()->get_setting( $merge_key, $device );
			$merge_next = false;
			$merge_prev = false;
			if ( 'no' == $merge || '0' == $merge ) {
				$merge = false;
			...
			if ( $next_item ) {
				...
				$merge_next     = Ebbe()->get_setting( $merge_key_next, $device );
			}
		...
		foreach ( $group_items as $item ) {
			...
			$classes = apply_filters( 'ebbe/builder/item-wrapper-classes', $classes, $item );
	...
	public function render( $row_ids = array( 'top', 'main', 'bottom' ) ) {
		...
		foreach ( $row_ids as $row_id ) {
			$show = ebbe_is_builder_row_display( $this->id, $row_id );
			if ( $show && isset( $this->rows[ $row_id ] ) ) {
				...
				if ( ! empty( $show_on_devices ) ) {
					...
					if ( ! empty( $desktop_items ) || ! empty( $mobile_items ) ) {
						...
						$row_layout    = Ebbe()->get_setting( $this->id . '_' . $row_id . '_layout' );
						$row_text_mode = Ebbe()->get_setting( $this->id . '_' . $row_id . '_text_mode' );
						if ( $row_layout ) {
							$classes[] = sanitize_text_field( $row_layout );
						}

						$classes = apply_filters( 'ebbe/builder/row-classes', $classes, $row_id, $this );

						$atts['class']       = join( ' ', $classes );
						$atts['id']          = 'cb-row--' . $_id;
						$atts['data-row-id'] = $row_id;
						$atts                = apply_filters( 'ebbe/builder/row-attrs', $atts, $row_id, $this );
						$string_atts         = '';
						foreach ( $atts as $k => $s ) {
							if ( is_array( $s ) ) {
								$s = wp_json_encode( $s );
						...
						$inner_class = apply_filters( 'ebbe/builder/inner-row-classes', $inner_class, $row_id, $this );
	...
	public function render_mobile_sidebar() {
		...
		$menu_sidebar_skin = Ebbe()->get_setting( 'header_sidebar_skin_mode' );
 ...
 * @deprecated 0.2.7
 */
class Ebbe_Customize_Layout_Builder_Frontend extends Ebbe_Layout_Builder_Frontend {}

/**
 * Alias of class Ebbe_Layout_Builder_Frontend
 *
 * @see Ebbe_Layout_Builder_Frontend
 *
 * @return Ebbe_Layout_Builder_Frontend
 */
function Ebbe_Layout_Builder_Frontend() {
	return Ebbe_Layout_Builder_Frontend::get_instance();
}
 ...
 * @return Ebbe_Layout_Builder_Frontend
 */
function Ebbe_Customize_Layout_Builder_Frontend() {
	return Ebbe_Layout_Builder_Frontend();
}
