 ...
 * @since 0.2.9
 */
function ebbe_maybe_change_header_version() {
 ...
 */
function ebbe_customize_render_header() {

	/**
	 * @since 0.2.9
	 */
	ebbe_maybe_change_header_version();
	$config = Ebbe_Customize_Layout_Builder()->get_builder( 'header' )->get_config();
	$control_id = 'header_builder_panel';
	$version = false;

	if ( isset( $config['version_id'] ) ) {
		$version = Ebbe()->get_setting( $config['version_id'] );
	}
	...
	$list_items = Ebbe_Customize_Layout_Builder()->get_builder_items( 'header' );

	$fn = 'Ebbe_Layout_Builder_Frontend';
	if ( $version ) {
	...
	/**
	 * @var Ebbe_Layout_Builder_Frontend $builder
	 */
	 ...
	 * @since 0.2.2
	 */
	do_action( 'ebbe/after-header' );
}

/**
 * Display Footer Layout
 */
function ebbe_customize_render_footer() {
	/**
	 * Hook before footer
	 *
	 * @since 0.2.2
	 */
	do_action( 'ebbe/before-footer' );
	if (Ebbe()->get_setting('ebbe_backtotop_status') == 1) {
    	get_template_part( 'templates/back-to-top/back-to-top');
    }
	echo '<footer class="site-footer" id="site-footer">';
	Ebbe_Customize_Layout_Builder_Frontend()->set_id( 'footer' );
	Ebbe_Customize_Layout_Builder_Frontend()->set_control_id( 'footer_builder_panel' );
	$list_items = Ebbe_Customize_Layout_Builder()->get_builder_items( 'footer' );
	Ebbe_Customize_Layout_Builder_Frontend()->set_config_items( $list_items );
	Ebbe_Customize_Layout_Builder_Frontend()->render();
	echo '</footer>';
	/**
	 * Hook before footer
	 *
