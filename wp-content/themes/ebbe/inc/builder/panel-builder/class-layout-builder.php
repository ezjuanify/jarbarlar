 ...
 *
 * Class Ebbe_Customize_Layout_Builder
 */
class Ebbe_Customize_Layout_Builder {
	static $_instance;
	...
	function init() {
		...
		do_action( 'ebbe/customize-builder/init' );

		if ( is_admin() ) {
			add_action( 'customize_controls_enqueue_scripts', array( $this, 'scripts' ) );
			add_action( 'customize_controls_print_footer_scripts', array( $this, 'template' ) );
			add_action( 'wp_ajax_ebbe_builder_save_template', array( $this, 'ajax_save_template' ) );
			add_action( 'wp_ajax_ebbe_builder_export_template', array( $this, 'ajax_export_template' ) );
		}
	 ...
	 *
	 * @see Ebbe_Customize_Builder_Panel
	 *
	 * @param string                            $id    ID of panel.
	 * @param Ebbe_Customize_Builder_Panel $class Panel class name.
	 *
	 * @return bool
	 */
	public function register_builder( $id, $class ) {
		...
		if ( ! $class instanceof Ebbe_Customize_Builder_Panel ) {
			$name = get_class( $class );
			_doing_it_wrong( $name, sprintf( esc_html__( 'Class <strong>%s</strong> do not extends class <strong>Ebbe_Customize_Builder_Panel</strong>.', 'ebbe' ), $name ), '1.0.0' );
			return false;
		}

		add_filter( 'ebbe/customizer/config', array( $class, '_customize' ), 35, 2 );
	 ...
	 *
	 * @see Ebbe_Customize_Layout_Builder::register_builder();
	 *
	...
	function get_builder_items( $builder_id ) {
		if ( ! $builder_id ) {
			return apply_filters( 'ebbe/builder/' . $builder_id . '/items', array() );
		}
		if ( ! isset( $this->registered_items[ $builder_id ] ) ) {
			return apply_filters( 'ebbe/builder/' . $builder_id . '/items', array() );
		}
		...
		$items = apply_filters( 'ebbe/builder/' . $builder_id . '/items', $items );
	...
	function ajax_save_template() {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( esc_html__( 'Access denied', 'ebbe' ) );
		}
		...
		if ( ! $save_name ) {
			$save_name = sprintf( esc_html__( 'Saved %s', 'ebbe' ), date_i18n( 'Y-m-d H:i:s' ) );
		}
		$fn = false;
		if ( ! isset( $this->registered_builders[ $id ] ) ) {
			wp_send_json_error( esc_html__( 'No Support', 'ebbe' ) );
		} else {
		...
		if ( ! $save_name ) {
			...
			$save_name = sprintf( esc_html__( 'Saved %s', 'ebbe' ), $key_id );
		} else {
			$key_id = $save_name;
		}
		...
		update_option( $option_name, $saved_templates );
		$html = '<li class="saved_template li-boxed" data-control-id="' . esc_attr( $control ) . '" data-id="' . esc_attr( $key_id ) . '" data-data="' . esc_attr( wp_json_encode( $new_template_data ) ) . '">' . esc_html( $save_name ) . ' <a href="#" class="load-tpl">' . esc_html__( 'Load', 'ebbe' ) . '</a><a href="#" class="remove-tpl">' . esc_html__( 'Remove', 'ebbe' ) . '</a></li>'; // WPCS: XSS OK.
		wp_send_json_success(
			array(
				'key_id' => $key_id,
				'name'   => $save_name,
	...
	public function get_builders() {
		...
		foreach ( $this->registered_builders as $id => $builder ) {
			...
			$config['items'] = apply_filters( 'ebbe/builder/' . $id . '/items', $this->get_builder_items( $id ) );
			$config['rows']  = apply_filters( 'ebbe/builder/' . $id . '/rows', $builder->get_rows_config() );
			$builders[ $id ] = $config;
		}
	...
	function scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'swicth_version'            => esc_html__( 'Switch Builder Version', 'ebbe' ),
				'hide_switcher'             => apply_filters( 'ebbe_hide_header_builder_switcher', get_theme_mod( 'hide_header_builder_switcher', 'no' ) ), // Use get theme mod `hide_header_builder_switcher` for hide switcher.
	...
	function template() {
		...
		?>
		<script type="text/html" id="tmpl-ebbe--builder-panel">
			<div class="ebbe--customize-builder">
				<div class="ebbe--cb-inner">
					<div class="ebbe--cb-header">
						<div class="ebbe--cb-devices-switcher">
						</div>
						<div class="ebbe--cb-actions">
							<?php do_action( 'ebbe/builder-panel/actions-buttons' ); ?>
							<a data-id="{{ data.id }}_templates" class="focus-section button button-secondary" href="#"><?php _e( 'Templates', 'ebbe' ); ?></a>
							<a class="button button-secondary ebbe--panel-close" href="#">
								<span class="close-text"><?php _e( 'Close', 'ebbe' ); ?></span>
								<span class="panel-name-text">{{ data.title }}</span>
							</a>
						</div>
					</div>
					<div class="ebbe--cb-body"></div>
				</div>
			</div>
		</script>


		<script type="text/html" id="tmpl-ebbe--cb-item">
			...
			>
				...
				<div class="grid-stack-item-content">
					<span class="ebbe--cb-item-name" data-section="{{ data.section }}">{{ data.name }}</span>
					<span class="ebbe--cb-item-remove ebbe-cb-icon"></span>
					<span class="ebbe--cb-item-setting ebbe-cb-icon" data-section="{{ data.section }}"></span>
				</div>
			</div>
		</script>
...
/**
 * Alias of class Ebbe_Customize_Layout_Builder
 *
 * @see Ebbe_Customize_Layout_Builder
 *
 * @return Ebbe_Customize_Layout_Builder
 */
function Ebbe_Customize_Layout_Builder() {
	return Ebbe_Customize_Layout_Builder::get_instance();
}
