 ...
 * @since 0.2.9
 */
class Ebbe_Layout_Builder_Frontend_V2  extends Ebbe_Abstract_Layout_Frontend {
	...
	public function render_item( $item_id, $col_id, $row_id, $device ) {
		 ...
		 * @since 0.2.1
		 */
		do_action( 'ebbe/builder/' . $this->id . '/before-item/' . $item_id );
		$object_item = Ebbe_Customize_Layout_Builder()->get_builder_item( $this->id, $item_id );
		 ...
		 * @since 0.2.1
		 */
		do_action( 'ebbe/builder/' . $this->id . '/after-item/' . $item_id );
			...
			if ( $show && isset( $this->rows[ $row_id ] ) ) {
				...
				if ( ! empty( $show_on_devices ) ) {
					...
					if ( ! empty( $desktop_row ) || ! empty( $mobile_row ) ) {
						...
						$align_classes = 'ebbe-grid-middle';
						...
						$show_on_sticky    = Ebbe()->get_setting( $this->id . '_' . $row_id . '_sticky_status' );
						if (!$show_on_sticky) {
							$classes[] = 'ebbe-hide-on-sticky';
						}
...
/**
 * Alias of class Ebbe_Layout_Builder_Frontend_V2
 *
 * @see Ebbe_Layout_Builder_Frontend
 *
 * @return Ebbe_Layout_Builder_Frontend_V2
 */
function Ebbe_Layout_Builder_Frontend_V2() {
	return Ebbe_Layout_Builder_Frontend_V2::get_instance();
}
