...
class Ebbe {
	 ...
	 * Main Ebbe Instance.
	 *
	 * Ensures only one instance of Ebbe is loaded or can be loaded.
	 *
	 * @return Ebbe Main instance.
	 */
	...
	function content_width() {
		$GLOBALS['content_width'] = apply_filters( 'ebbe_content_width', 843 );
	}
	...
	function scripts() {
		...
		if ( ! class_exists( 'Ebbe_Font_Icons' ) ) {
			require_once get_template_directory() . '/inc/builder/customizer/class-customizer-icons.php';
		}
		Ebbe_Font_Icons::get_instance()->enqueue();

		$suffix = $this->get_asset_suffix();

		do_action( 'ebbe/load-scripts' );

		$css_files = apply_filters(
			'ebbe/theme/css',
			array(
				'google-font' => Ebbe_Customizer_Auto_CSS::get_instance()->get_font_url(),
		...
		foreach ( $css_files as $id => $url ) {
			...
			if ( is_array( $url ) ) {
				...
				wp_enqueue_style( 'ebbe-' . $id, $arg['url'], $arg['deps'], $arg['ver'] );
			} elseif ( $url ) {
				wp_enqueue_style( 'ebbe-' . $id, $url, $deps, self::$version );
			}
		}
		...
		wp_add_inline_style( 'ebbe-style', Ebbe_Customizer_Auto_CSS::get_instance()->auto_css() );
		wp_localize_script(
			'ebbe-themejs',
			'Ebbe_JS',
			apply_filters( // phpcs:ignore
				...
				array(
					'is_rtl'                     => is_rtl(),
					'css_media_queries'          => Ebbe_Customizer_Auto_CSS::get_instance()->media_queries,
					'sidebar_menu_no_duplicator' => Ebbe()->get_setting( 'header_sidebar_menu_no_duplicator' ),
				)
			)
		);

		do_action( 'ebbe/theme/scripts' );
	}
	...
	function init() {
		$this->init_hooks();
		$this->includes();
		$this->customizer = Ebbe_Customizer::get_instance();
		$this->customizer->init();
		do_action( 'ebbe/init' );
	}

	function get_setting( $id, $device = 'desktop', $key = null ) {
		return Ebbe_Customizer::get_instance()->get_setting( $id, $device, $key );
	}

	function get_media( $value, $size = null ) {
		return Ebbe_Customizer::get_instance()->get_media( $value, $size );
	}

	function get_setting_tab( $name, $tab = null ) {
		return Ebbe_Customizer::get_instance()->get_setting_tab( $name, $tab );
	}
