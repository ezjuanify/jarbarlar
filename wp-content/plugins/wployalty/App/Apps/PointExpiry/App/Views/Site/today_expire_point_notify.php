...
if ( ! empty( $today_expire_point ) ): ?>
    <div class="wlr-today-expire-points-section wlr-border-color">
        ...
        <p class="wlrborder wlr-text-color"
           style="margin: 0;"><?php echo sprintf( __( 'Alert! Your %d %s expire today.Use them now to avoid missing out.', 'wp-loyalty-rules' ), $today_expire_point, $earn_campaign_helper->getPointLabel( $today_expire_point ) ); ?></p>
		<?php if ( isset( $show_redeem ) && $show_redeem ): ?>
            <p onclick="wlr_jquery( 'body' ).trigger( 'wlr_my_reward_section',[ 'rewards'])"
               ...
               style="margin: 0;text-decoration: none;"><?php _e( 'Redeem Now!', 'wp-loyalty-rules' ); ?></p>
		<?php endif; ?>
    </div>
<?php endif; ?>
